...
export function SocialAuth({
  ...
  loadingProvider: externalLoadingProvider,
  className = "",
}: SocialAuthProps) {
  const socialLoginHook = useSocialLogin();

  const isLoading = externalIsLoading ?? socialLoginHook.isLoading;
  const loadingProvider = externalLoadingProvider ?? socialLoginHook.loadingProvider;
  ...
  const handleProviderClick = (provider: string) => {
    ...
    if (onSocialLogin) {
      ...
      socialLoginHook.handleSocialLogin(provider);
    }
  };

  const isGoogleLoading = isLoading && loadingProvider === "google";
  const isGithubLoading = isLoading && loadingProvider === "github";

  return (
    <div className={`grid grid-cols-2 gap-3 select-none ${className}`}>
      {/* Google OAuth Button */}
      <button
        type="button"
        disabled={isLoading}
      ...
      >
        ...
        <span>{isGithubLoading ? "Connecting..." : "Continue with GitHub"}</span>
      </button>
    </div>
  );
}
